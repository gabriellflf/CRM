...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha válida encontrada. Certifique-se de que o CSV possui o cabeçalho "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} atribuição${tagsAssigned !== 1 ? 'ões' : ''} de tag aplicada${tagsAssigned !== 1 ? 's' : ''}`
        );
      }
      ...
      if (failed > 0) {
        toast.error(
          `${failed} contato${failed !== 1 ? 's' : ''} falhou${failed !== 1 ? 'ram' : ''} na importação`
        );
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Importação falhou';
      toast.error(message);
    } finally {
      setImporting(false);
    }
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground uppercase">
                  Pré-visualização · primeiros {preview.length}
                </p>
