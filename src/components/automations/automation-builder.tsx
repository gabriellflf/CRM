...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  { value: "new_message_received", label: "Nova mensagem recebida", hint: "Qualquer mensagem recebida" },
  {
    ...
    hint: "Primeira vez que este contato envia mensagem (funciona com contatos adicionados manualmente também)",
  },
...
function SendTemplateFields({
  ...
  return (
    <FieldBlock label="Template de mensagem">
      <select
        value={current}
        onChange={(e) => {
    ...
    <div className="space-y-2">
      <div>
        ...
        <Input
          ...
          placeholder="ex.: preço, pedido de demo, falar com vendas"
          className="bg-muted text-foreground"
        />
      </div>
      <div>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {isCondition ? "Condição" : step.step_type === "wait" ? "Esperar" : "Ação"}
              </div>
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case "send_template":
      return (step.step_config.template_name as string) || "escolher um template"
    case "wait":
