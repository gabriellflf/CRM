...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    if (!lastCustomerMsg) return { expired: true, remaining: "Sem msgs do cliente" };
  ...
  const handleSendTemplate = useCallback(
    async (
      ...
      try {
        ...
        if (!res.ok) {
          const reason = payload?.error || `HTTP ${res.status}`;
          console.error("[template-send] ERRO COMPLETO:", { reason, payload, status: res.status });
          toast.error(`Falha ao enviar modelo: ${reason}`, { duration: 12000 });
          onUpdateMessage(tempId, { status: "failed" });
          return;
        }
    ...
    return (
      <div className={cn("flex flex-1 flex-col items-center justify-center", DOODLE_BG_CLASSES)}>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Escolha uma conversa à esquerda para começar a mensagear
        </p>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={cn(
                ...
                {currentStatus?.label ?? "Status"}{/* pt-BR via STATUS_OPTIONS */}
