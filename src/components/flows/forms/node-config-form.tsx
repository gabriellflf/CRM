    ...
    case "end":
      return (
        <p className="text-xs text-muted-foreground">
          Nó terminal. Quando o executor chega aqui, a execução é marcada como concluída. Nenhuma configuração necessária.
        </p>
      );
  }
}
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <div className="md:col-span-2">
          ...
          {subject === "tag" && tags.length > 0 ? (
            ...
            >
              <SelectTrigger className="bg-muted">
                <SelectValue placeholder="Escolher uma tag…" />
              </SelectTrigger>
              <SelectContent>
                {tags.map((t) => (
            ...
            >
              <SelectTrigger className="bg-muted">
                <SelectValue placeholder="Escolher um campo…" />
              </SelectTrigger>
              <SelectContent>
  ...
  const handleFile = useCallback(
    async (file: File) => {
      if (file.size > MEDIA_MAX_BYTES) {
        toast.error(
          `Arquivo tem ${(file.size / 1024 / 1024).toFixed(1)} MB — limite é 16 MB.`,
        );
        return;
      }
      setUploading(true);
